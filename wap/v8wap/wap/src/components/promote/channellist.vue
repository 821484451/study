      ...
      <div class="searchContent">
        <ul class="searchContent_ul" >
          <li class="searchContent_ul__li" v-for="(item, index) in channelList" :key="index" >
            ...
            <div class="channelTime">{{$dataFormat('yyyy-MM-dd hh:mm:ss',item.create_time)}}</div>
          </li>
        </ul>
      </div>
      ...
      if (niceName && userName && password) {
        if (reg1.test(userName)) {
          if (reg2.test(password)) {
            store.dispatch('addChannel', {nicename: this.agentName, agentname: this.agentUser, password: this.agentPass})
            this.popAC()
            this.agentName = ''
            this.agentUser = ''
            this.agentPass = ''
