<template>
  <div class="gameList">
    ...
    <ul class="gameList_ul" v-for="(item, index) in gameList" :key="index">
      ...
      <li class="proportion">
        <div >
          ...
          <p>返利比例：<span>{{item.sub_agent_rate}}折</span></p>
          <p>优惠类型：<span>{{item.sub_agent_rate}}折</span></p>
        </div>
      </li>
    </ul>
  </div>
...
export default {
  ...
  created () {
    ...
    store.dispatch('gameList', {'classify': 3})
  },
  methods: {
    popAdd (text, id) {
