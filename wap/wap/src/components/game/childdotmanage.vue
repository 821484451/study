      ...
      <ul class="myGame_ul" :key="index">
        <li class="myGame_ul__li">
          ...
          <div class="gameDetail">
            ...
            <p class="gameDetail_p3"><span class="discounts">{{item.sub_name}}</span></p>
          </div>
          <div class="gameBtn"><a href="javascript:;" @click="popBox(item.child_agent_rate, item.mem_first, item.mem_offer, item.ag_id)">修改</a></div>
        </li>
        <li class="proportion">
          <div>
            <p>折扣：<span>{{item.child_agent_rate}}折</span></p>
            <p>发币折扣：
              <span>{{item.child_grant_rate}}折</span>
            </p>
          </div>
          <div>
